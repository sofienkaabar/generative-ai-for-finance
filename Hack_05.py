...
synthetic_data = synthetic_data * (np.max(data) - np.min(data)) + np.min(data)
synthetic_data = np.transpose(synthetic_data)
# Aggregate the synthetic data
mean = np.mean(synthetic_data, axis = 1)
# Plot the original and synthetic data
fig, axs = plt.subplots(2, 1)
axs[0].plot(data, label = 'Original Data', color = 'black')
axs[0].legend()
axs[0].grid()
plt.plot(mean, label = 'Aggregated Synthetic Data', linewidth = 1)
plt.xlabel('Time Steps')
plt.ylabel('Value')
plt.title('Original (black) vs. Synthetic Time Series Data (blue)')
plt.show()
plt.legend()
plt.grid()